...
class P2M2_CDR(nn.Module):
    def __init__(self,**params):
        ...
        self.beta = 0.001
        self.noise_control = params['ldp_noise']
        self.sensitivity = 1.0
        self.epsilon = 1.0
        self.disen_agg_way = params['agg_way']
        ...
        self.source_disen_concat_layer = nn.Linear(self.domain_disen_dim * 2, self.domain_disen_dim).to(self.device)
        self.target_disen_concat_layer = nn.Linear(self.domain_disen_dim * 2, self.domain_disen_dim).to(self.device)
    ...
    def forward(self, source_nodes_u, source_nodes_v,target_nodes_u,target_nodes_v):
        ...
        if self.disen_agg_way == 'sum':
            source_u_feats = source_disen_feats_c+source_disen_feats_s
            target_u_feats = target_disen_feats_c+target_disen_feats_s
        elif self.disen_agg_way == 'avg':
            source_u_feats = (source_disen_feats_c + source_disen_feats_s)/2
            target_u_feats = (target_disen_feats_c + target_disen_feats_s)/2
        else:
            source_u_feats = F.relu(
                self.source_disen_concat_layer(torch.cat([source_disen_feats_c, source_disen_feats_s], dim=1)))
            target_u_feats = F.relu(
                self.target_disen_concat_layer(torch.cat([target_disen_feats_c, target_disen_feats_s], dim=1)))
